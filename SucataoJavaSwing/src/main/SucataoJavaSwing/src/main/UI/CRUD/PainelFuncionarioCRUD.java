...
public class PainelFuncionarioCRUD extends JPanel {
    ...
    public PainelFuncionarioCRUD(TelaPrincipal janela) {
        ...
        // MOSTRAR FUNCIONARIO NOS CAMPOS AO CLICAR
        table.getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) { // Prever duplo-clique
                int linhaSelecionada = table.getSelectedRow();
                if (linhaSelecionada != -1) {
                    indexSendoEditado = linhaSelecionada;
                    Funcionario f = listaMemoria.get(indexSendoEditado);
                    
                    // Preenche os campos com a info selecionada
        ...
        btnAtt.addActionListener(e -> {
            if (indexSendoEditado == -1) {
                JOptionPane.showMessageDialog(this, "Por favor, selecione um funcionário na tabela primeiro para poder atualizar.");
                return;
            }
            ...
            if (nome.isEmpty() || email.isEmpty() || cargo.isEmpty() || cargo.equals("Selecione...")) {
                JOptionPane.showMessageDialog(this, "Campos inválidos ou em branco. Verifique os dados fornecidos.");
                return;
            }
            
            int arroba = email.indexOf("@");
            if (arroba == -1 || email.indexOf(".", arroba) == -1) {
                JOptionPane.showMessageDialog(this, "Endereço de e-mail com formato inválido.", "Erro de Validação", JOptionPane.WARNING_MESSAGE);
                return;
            }
            
            try {
                // colocar o objeto na memoria cache
                Funcionario f = listaMemoria.get(indexSendoEditado);
                
                // Mudar os valores utilizando os setters padrao
                f.setNome(nome);
                f.setEmail(email);
                f.setCargo(cargo);
                
                // Salvar o Json com os valores mudados
                HandlerBD.salvarFuncionarios(listaMemoria);
                
                atualizarTableGeral();
                limparFormulario();
                JOptionPane.showMessageDialog(this, "Dados do funcionário atualizados com sucesso!");
            } catch (RuntimeException ex) {
